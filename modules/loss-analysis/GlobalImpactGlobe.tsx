...
const GLOBAL_DATASET: CountryData[] = [
  ...
  { id: "BRA", name: "Brazil", loss: 16.0, status: "OPORTUNIDAD", insight: "Mercado gigante. 40% de pérdidas en zonas de favelas.", potential: "$420M", flag: "https://flagcdn.com/w80/br.png", coordinates: [-51.92, -14.23] },
  ...
  { id: "HTI", name: "Haiti", loss: 60.0, status: "CRÍTICO", insight: "Colapso de red. Prioridad: Detección masiva de fraude.", potential: "$12M", flag: "https://flagcdn.com/w80/ht.png", coordinates: [-72.28, 18.97] },
  ...
  { id: "PER", name: "Peru", loss: 10.0, status: "EFICIENTE", insight: "Caso de éxito tras reforma regulatoria.", potential: "$45M", flag: "https://flagcdn.com/w80/pe.png", coordinates: [-75.01, -9.19] },
...
const colorScale = scaleLinear<string>()
  .domain([0, 10, 20, 60])
  .range(["#1E293B", "#30677E", "#F48423", "#EF4444"]);
...
export const GlobalImpactGlobe: React.FC = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full w-full bg-slate-900 border border-white/5 overflow-hidden min-h-[500px]">
      ...
      {/* PANEL IZQUIERDO: GLOBO + MINI CARDS */}
        ...
        <div className="flex-1 relative">
          ...
          >
            ...
            <Geographies geography={geoUrl}>
              {({ geographies }) =>
                geographies.map((geo) => {
                  // Lógica de emparejamiento mejorada para asegurar el resaltado
                  const countryName = geo.properties.name;
                  const isoA3 = geo.properties.ISO_A3 || geo.properties.iso_a3;
                  
                  // Intentamos encontrar el país por nombre o por código ISO
                  const match = GLOBAL_DATASET.find(d => 
                    d.id === isoA3 || 
                    d.name === countryName ||
                    (countryName === "Brazil" && d.id === "BRA") || // Hard fixes para inconsistencias de nombres
                    (countryName === "Colombia" && d.id === "COL") ||
                    (countryName === "Venezuela" && d.id === "VEN")
                  );
                  ...
                  return (
                    <Geography
                      key={geo.rsmKey}
                      geography={geo}
                      fill={isActive ? BRAND_ORANGE_ACTIVE : (match ? colorScale(match.loss) : "#0f172a")}
                      stroke={isActive ? "#ffffff" : "#030712"}
                      strokeWidth={isActive ? 1.0 : 0.2}
                      style={{
                        default: { 
                          outline: 'none', 
                        ...
                        hover: { 
                          fill: isActive ? BRAND_ORANGE_ACTIVE : (match ? "#334155" : "#0f172a"),
                          outline: 'none', 
                      ...
                      onClick={() => {
                        if (match) {
                          const index = GLOBAL_DATASET.findIndex(d => d.id === match.id);
            ...
            {GLOBAL_DATASET.map((data, idx) => {
              ...
              return (
                <Marker key={data.id} coordinates={data.coordinates}>
                  {isActive && (
                    <motion.circle
                      r={windowWidth < 640 ? 12 : 15}
                      fill="none"
                      stroke={BRAND_ORANGE_ACTIVE}
                      strokeWidth={2}
                  ...
                  <circle
                    ...
                    stroke={isActive ? BRAND_ORANGE_ACTIVE : "#ffffff"}
          ...
          <div className="absolute bottom-4 left-4 sm:bottom-6 sm:left-6 z-20">
             <AnimatePresence mode="wait">
               <motion.div
                 ...
                 exit={{ opacity: 0, y: -5 }}
                 className="flex items-center gap-2 bg-slate-900/95 backdrop-blur-md border border-white/10 p-2 sm:p-2.5 rounded-lg sm:rounded-xl shadow-2xl"
               >
                  <img src={activeCountry.flag} className="w-6 h-4 sm:w-8 sm:h-5 rounded shadow-sm object-cover" />
        ...
        <div className="px-3 sm:px-6 pb-4 sm:pb-6 relative z-30">
          <div className="grid grid-cols-2 gap-2 sm:gap-4">
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              className="bg-slate-900/50 backdrop-blur-md border border-white/10 p-2.5 sm:p-3 rounded-xl sm:rounded-2xl"
            >
              <span className="text-[7px] sm:text-[9px] text-slate-500 font-bold uppercase mb-0.5 block">Pérdidas NTL</span>
            ...
            <motion.div 
              ...
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-slate-900/50 backdrop-blur-md border border-brand-orange/20 p-2.5 sm:p-3 rounded-xl sm:rounded-2xl"
            >
              <span className="text-[7px] sm:text-[9px] text-brand-orange font-bold uppercase mb-0.5 block">Recuperable</span>
      ...
      <div className="flex-1 flex flex-col bg-slate-900 h-[50%] lg:h-full lg:w-[450px] shrink-0">
        <div className="flex items-center gap-1.5 p-2 sm:p-4 bg-slate-950/50 border-b border-white/5 overflow-x-auto no-scrollbar scroll-smooth">
          {GLOBAL_DATASET.map((data, idx) => (
            <button
              ...
              className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-[8px] sm:text-[10px] font-bold uppercase tracking-widest transition-all ${
                idx === currentIndex 
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-5 sm:p-8">
          <AnimatePresence mode="wait">
            ...
            >
              <div>
                 <span className="text-[8px] sm:text-[9px] text-brand-blue font-black tracking-widest uppercase mb-1 block">Análisis Local</span>
                 <h2 className="text-xl sm:text-3xl font-black text-white tracking-tighter leading-none mb-2">{activeCountry.name}</h2>
                 <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[8px] sm:text-[9px] font-black uppercase tracking-wider ${
                    ...
                    {activeCountry.status === 'CRÍTICO' ? <AlertCircle className="w-3 h-3" /> : 
                     activeCountry.status === 'OPORTUNIDAD' ? <TrendingDown className="w-3 h-3" /> : <CheckCircle2 className="w-3 h-3" />}
        ...
        <div className="p-4 sm:p-6 bg-slate-950 border-t border-white/5">
           <div className="flex items-center justify-between">
              <div className="flex gap-2.5">
                 <button 
                   onClick={handlePrev}
                   className="w-10 h-10 sm:w-11 sm:h-11 rounded-lg bg-slate-800 hover:bg-brand-blue transition-all flex items-center justify-center group"
                 >
              ...
              <div className="text-right">
                 <div className="text-[7px] sm:text-[9px] text-slate-500 font-bold uppercase tracking-widest mb-1">Índice Global {currentIndex + 1} / {GLOBAL_DATASET.length}</div>
                 <div className="flex gap-1.5 justify-end">
                    {GLOBAL_DATASET.map((_, i) => (
                      <div 
                        key={i} 
                        className={`h-1 rounded-full transition-all duration-300 ${i === currentIndex ? 'w-5 sm:w-7 bg-brand-orange' : 'w-1 sm:w-1.5 bg-slate-800'}`} 
                      />
                    ))}
                 </div>
              </div>
